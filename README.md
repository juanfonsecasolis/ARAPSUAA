...
**Results.** The best detection rates reached in this work were 93% precision, 89% specificity, 92% recall, 92% F-score, and 80% Matthew's correlation coefficient. So we reached an improvement of +12% in recall, meaning that the number of true positives increased, while keeping almost constant the number of false positives &mdash;important for user's safety&mdash;. Also, the covariance matrices reached acceptable metrics, confirming the identification of noisy patterns using this approach is possible.
