...
The recordings were acquired in 2013 by Mario Monge and Sharon Bejarano using two smartphones of medium-gamma (Samsung Galaxy Ace S5830 and Samsung Galaxy S Duos S7562). Later on, new recordings were taken using a LG G2 mini D618. Originally, the audios were taken using a sampling rate of 44.1 kHz and a quantization of 32 bits, but then downsampled to 22.05 kHz to save disk space as no relevant information is stored above that frequency range. The information was captured either during the morning, when less traffic was present in streets, as well as after midday, when roads were more populated. The weather conditions were mostly sunny and cloudly, and at least one was taken with presence of light rain. The duration of the recordings ranged from 13 s to 35 s, depending on the activity period of the Novax units, and the signal-to-noise ratio (SNR) corresponded in average to 1.1 dB (Novax units were designed to maintain an SPL level of +5dB above the ambient noise, with a limit on 90 dB).
...
Disclaimer
---
Although I received no grant or scholarship to create ARAPSUAA, I took as basis the theory publicly published by the University of Costa Rica (UCR) in the paper [Automatic recognition of accessible pedestrian signals](https://doi.org/10.1121/2.0000675) (Journal of the Acoustical Society of America; 141-3913; Boston, USA; 2017). This paper was published as part of the project [B6146](https://vinv.ucr.ac.cr/sigpro/web/projects/B6146) where I worked as a researcher to develop an Android Application called _Reconocimiento Acústico de Señales Peatonales_ (RASP). The code of the application belongs to the UCR, is private, and was written in Java; in an opposite way, the code of ARAPSUAA was written completely in Python, incorporates the improvements mentioned in the abstract (new kernel, adaptive threshold, and covariance matrixes), and serves as a scientific library. I have made public the code of ARAPSSUAA for the sake of reproducible research, moreover, because I want to encourage researchers living in low-income-countries (LIC) to perform independent research when working full-time in the academia is not possible.
